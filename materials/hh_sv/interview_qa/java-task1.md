...
1. Java heap space (самый частый)
   ...
   **Причины:**
    ...
    - Memory Leak — 
        это ситуация, при которой в памяти приложения остаются объекты, 
        которые больше не используются логикой программы, но сборщик мусора (Garbage 
        Collector / GC) не может их удалить, потому что на них всё еще существуют 
        живые ссылки. Объекты продолжают жить в памяти, хотя уже не нужны (самая
...
- Неправильное использование static полей
    Суть: Статические переменные привязаны к классу и живут всё время, пока работает 
    приложение. Если вы случайно запишете миллион объектов в static List, они никогда не удалятся.
- Забытые слушатели и колбэки (Listeners / Observers)
    Суть: Если вы регистрируете объект в каком-то долгоживущем синглтоне как слушатель событий, 
    но забываете вызвать removeListener(), этот синглтон будет вечно держать объект в памяти.
- Использование ThreadLocal без очистки
    Суть: Данные в ThreadLocal привязаны к потоку. В веб-серверах (Tomcat, Netty) потоки 
    переиспользуются в пуле (Thread Pool) и никогда не умирают. Если в конце обработки запроса 
    не вызвать threadLocal.remove(), данные зависнут в памяти навсегда.
- Ошибки в hashCode() и equals() при работе с хэш-мапами
    Суть: Если вы добавили объект в HashMap или HashSet, а затем изменили поле, которое участвует 
    в расчете hashCode(), вы больше не сможете найти, удалить или перезаписать этот объект. Он останется 
    в мапе мертвым грузом.

Как это находить и чинить (Ответ для собеседования)


Если на собеседовании спросят: «Приложение падает по OOM, ваши действия?», отвечайте по шагам:
1. Анализ графиков: Смотрим на графики Prometheus/Grafana (метрика jvm_memory_used_bytes). При утечке 
график превращается в «пилу», у которой нижние точки (после GC) с каждым разом становятся всё выше и выше, 
пока не упрутся в лимит.
2. Снятие дампа: Делаем снимок памяти (Heap Dump) работающего приложения с помощью утилиты jcmd, jmap или флага -XX:+HeapDumpOnOutOfMemoryError.
3. Анализ дампа: Открываем файл в анализаторе (например, Eclipse MAT или Profiler в IntelliJ IDEA).
4. Поиск виновника: Ищем вкладку Leak Suspects. Смотрим, какие классы занимают больше всего места и по 
какой цепочке ссылок (Path to GC Roots) они удерживаются в памяти.
...
2. GC Overhead limit exceeded
   ...
   **Сообщение:** `java.lang.OutOfMemoryError: GC overhead limit exceeded`
   ...
   Решения:
    ...
    - Попробовать другой GC: G1 (по умолчанию), ZGC или Shenandoah (для низких пауз).
...
итоге к `OutOfMemoryError: Java heap space`.
...
2. Увеличение лимитов ОС (временное решение)
   ...
   Рекомендации по архитектуре
    ...
    - Избегай создания потоков вручную через `new Thread()` или
      `ThreadPoolExecutor` с большим количеством потоков.
...
просто не запускается, и при ошибке данные не откатываются.
...
Это ситуация, когда ты вызываешь один метод из другого внутри того же самого класса.
...
напрямую через this, минуя Прокси. А раз Прокси не задействован, то и команду 
BEGIN TRANSACTION дать некому. Аннотация на методе saveData в этот момент — просто бесполезный текст.

Как исправить:

* Вынести saveData в другой сервис.
* Или вызывать createOrder так, чтобы транзакция начиналась на нем самом (
  повесить @Transactional на createOrder).

Проблема №2: Только public-методы

Spring умеет вешать транзакции только на public методы. Если ты напишешь:

@Transactional private void save() { ... } // ТРАНЗАКЦИЯ НЕ ЗАПУСТИТСЯ
...
1. Проблема видимости (Visibility) и кэши процессора
   ...
   процессора), куда он копирует переменные из основной памяти для скорости.
    ...
    * Связь: Если два потока меняют одну переменную, они могут делать это в
      ...
      локальная копия в памяти (кэше) не обновилась.
    ...
    * Решение: Ключевое слово volatile. Оно заставляет потоки всегда читать и
      писать напрямую в основную память (RAM), минуя локальные кэши.
...
* Пример с HashMap: Обычный HashMap не потокобезопасен. Если два потока 
    одновременно вызывают put(), может произойти повреждение внутренней 
    структуры данных (например, возникнет кольцевая ссылка в связном списке 
    корзины).
* Результат:
    ...
    2. Объекты в этой корзине никогда не будут удалены сборщиком мусора, так как
     они ссылаются друг на друга по кругу. Это ведет к OutOfMemoryError.

3. Создание «мусора» (Object Allocation Rate)
   Когда из-за состояния гонки потоки вынуждены постоянно перезаписывать данные
   или бесконечно повторять операции (в случае оптимистических блокировок).

* Сценарий: Если в коде используется паттерн compare-and-swap (CAS) и за
  переменную идет огромная конкуренция, сотни потоков будут постоянно создавать
  новые объекты-результаты или обертки, которые тут же будут отбрасываться при 
  неудачной попытке обновления.
* Связь: Это резко повышает Allocation Rate (скорость создания объектов).
  Сборщик мусора (GC) начинает работать слишком часто, пытаясь вычистить 
  этот «мусорный» поток объектов, что приводит к деградации производительности 
  всего приложения.

4. Небезопасная публикация (Safe Publication)

Если один поток создает объект, а второй пытается его прочитать в тот момент,
когда объект еще «не до конца» инициализирован (из-за переупорядочивания 
инструкций процессором).

* Связь: Поток Б может увидеть поля объекта со значениями по умолчанию (
  например, null или 0), хотя конструктор в Потоке А уже вроде как отработал. 
  Это часто приводит к NullPointerException в самом неожиданном месте.

Резюме для ответа:

«Race Condition напрямую влияет на память через модель памяти Java (JMM). Она
вызывает проблемы видимости данных между кэшами процессоров и может приводить 
к повреждению структур данных (как в случае с HashMap), что вызывает утечки 
памяти или бесконечные циклы, блокирующие сборщик мусора».
...
которую ждёт другой поток, и при этом ждёт блокировку, которую держит первый.
